...
boxersData.forEach((boxer, index) => {
  ...
  boxerIndex.push({
    ...boxerMetadata,
    slug,
    hasBouts: !!bouts && (typeof bouts === 'string' ? JSON.parse(bouts).length > 0 : bouts.length > 0),
    hasBio: !!bio
  });
...
Object.entries(divisions).forEach(([divisionSlug, boxers]) => {
  fs.writeFileSync(
    path.join(outputDir, `division-${divisionSlug}.json`),
    JSON.stringify(boxers)
  );
});
