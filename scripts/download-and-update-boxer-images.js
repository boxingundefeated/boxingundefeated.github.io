...
const boxersDataPath = path.join(__dirname, '../data/boxers.json');
...
async function processBoxers() {
  ...
  for (let i = 0; i < boxersData.length; i++) {
    ...
    const slug = boxer.slug || boxer.name.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '');
    ...
    try {
      ...
      if ((i + 1) % 50 === 0) {
        console.log(`Progress: ${i + 1}/${boxersData.length} - Downloaded: ${downloaded}, Failed: ${failed}, Skipped: ${skipped}`);
      }
